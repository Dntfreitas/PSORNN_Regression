milk=readtable('original/milk.csv'); %Load the excel file

% X(t) Extraction
x=milk(:,[1,2]); % X(t) Extraction
x=x{:,:}; % Changing the table format to matrix for X(t)

%  Y(t) Extraction
y=milk(:,end); % Y(t) Extraction
y=y{:,:};% Changing the table format to matrix for Y(t)
...
num = 1:length(Trend);
export = [x  Trend];
writematrix(export, 'dess/milk.csv');